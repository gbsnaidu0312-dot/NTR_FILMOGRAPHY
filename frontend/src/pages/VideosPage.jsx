...
export const VideosPage = () => {
  ...
  const fetchAndBuildFolders = async () => {
    try {
      setLoading(true);
      const type = activeType === 'events' ? 'event' : 'celebration';
      ...
      results.forEach(video => {
        const name = video.folder_name || 'General';
        if (!seen.has(name)) {
          seen.add(name);
          const count = results.filter(v => (v.folder_name || 'General') === name).length;
  ...
  const fetchVideosForFolder = (folder) => {
    const filtered = allVideos.filter(v => (v.folder_name || 'General') === folder.name);
    setVideos(filtered);
          ...
          {!activeType ? (
            ...
            >
              ...
              {folders.length > 0 ? (
                ...
                <EmptyState title="No Folders Found" description="No folders available yet" />
              )}
            </motion.div>
          ) : (
            ...
            >
              ...
              {videos.length > 0 ? (
                ...
                <EmptyState
                  ...
                  description="No videos available in this folder yet"
                />
              )}
            </motion.div>
          )}
